...
To visualize these schemas, \noborderacrshort{uml} diagram is provided in \autoref{fig:data-schema}. The diagram provides a general overview of utilized structures and associations; however, the implementation requires slight adjustments.
...
\texttt{UserCreation} corresponds to the configuration entity of the domain model but has been renamed in the implementation, as \textquote{configuration} is an ambiguous term that can mean several things in application development (e.g. configuration of the application itself). This naming clarifies what this data schema represents.
...
\captionsource{Screenshot of configuration screen}{3D assets used within the interface are adapted from~\cite{3dModel}}
...
\captionsource{Screenshot of introduction screen}{3D assets used within the interface are adapted from~\cite{3dModel}}
...
\captionsource{Screenshot of confirmation screen}{3D assets used within the interface are adapted from~\cite{3dModel}}
