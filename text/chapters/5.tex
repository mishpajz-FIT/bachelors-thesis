...
To streamline this manual testing process, automatic deployment of the development environment was enabled using the GitLab \noborderacrshort{ci}/\noborderacrshort{cd} pipeline, which was triggered after each change to the codebase. To assess various different functionalities of the application, several different products, such as computer or shelves configurations, were created to enable the system testing process. 
...
A score is then calculated from the \noborderacrshort{sus} questionnaire in as follows:
...
The average \noborderacrshort{sus} score for all participants was 77 which is considered good and above average when compared to other systems. The lowest \noborderacrshort{sus} score was 65, recorded for Participant \hyperref[itm:D]{D}, and the highest was 92.5, recorded for Participant \hyperref[itm:B]{B}. The scores for all participants are plotted on the chart shown in~\autoref{fig:sus-scores}.
